...
if __name__ == "__main__":
    ...
    if args.run_distribute:
        if target == "Ascend":
            ...
            context.set_context(device_id=device_id, enable_asuto_mixed_precision=True)
            ...
            config.rank = get_rank()
            config.group_size = get_group_size()
            context.set_auto_parallel_context(device_num=1, parallel_mode=ParallelMode.DATA_PARALLEL,
                                              gradients_mean=True)
    else:
        try:
            device_id = int(os.getenv('DEVICE_ID'))
    ...
    if args.is_model_arts:
        ...
        dataset = create_dataset(train_dataset_path,  do_train=True, repeat_num=1,
                             batch_size=64, train_image_size=224,
                             eval_image_size=config.eval_image_size, target="GPU",
                             distribute=config.run_distribute)
    else:
        train_dataset_path = '/data/lql/mscode/data/ILSVRC2012/train'
        dataset = create_dataset(train_dataset_path,  do_train=False, repeat_num=1,
                             batch_size=64, train_image_size=224,
                             eval_image_size=224, target="GPU",
                             distribute=False)#config.run_distribute)
    ...
    if args.run_eval:
        ...
        eval_dataset = create_dataset(path, do_train=False, repeat_num=1,
                                 ...
                                 eval_image_size=224, target="GPU",
                                 distribute=False)
        # eval_dataset = ImageNet(path, mode="val",
    ...
    context.set_context(mode=context.PYNATIVE_MODE)
    model.train(config.epochs, dataset, callbacks=callbacks, dataset_sink_mode=False)
